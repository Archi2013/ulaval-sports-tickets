...
public class PaymentServiceTest {
	...
	public void setUp() throws Exception {
		/*
		 * chooseTicketsVM = mock(ChosenTicketsViewModel.class); selectedSeats =
		 * new ArrayList<>(); selectedSeats.add("X3"); selectedSeats.add("X7");
		 * when(chooseTicketsVM.getSectionName()).thenReturn(SECTION_NAME);
		 * when(chooseTicketsVM.getNumberOfTicketsToBuy()).thenReturn(
		 * NUMBER_OF_TICKETS_TO_BUY);
	...
	/*
	 * @Test public void
	 * given_a_ChooseTicketsViewModel_getPayableItemsViewModel_should_return_a_PayableItemsViewModel
	 * () throws GameDoesntExistException, SectionDoesntExistException { GameDto
	 * gameDto = mock(GameDto.class); SectionDto sectionDto =
	 * mock(SectionDto.class); PayableItemsViewModel payableItemsVM =
	 * mock(PayableItemsViewModel.class);
	 * 
	 * when(gameDao.get(GAME_ID)).thenReturn(gameDto);
	 * when(sectionDao.getAvailable(GAME_ID,
	 * SECTION_NAME)).thenReturn(sectionDto);
	 * when(payableItemsViewModelFactory.createViewModel(chooseTicketsVM,
	 * gameDto, sectionDto)).thenReturn(payableItemsVM);
	 * 
	 * PayableItemsViewModel actual =
	 * paymentService.getPayableItemsViewModel(chooseTicketsVM);
	 * 
	 * assertSame(payableItemsVM, actual); }
	 * 
	 * @Test(expected = SectionDoesntExistException.class) public void
	 * given_a_ChooseTicketsViewModel_getPayableItemsViewModel_should_throw_SectionDoesntExistException_when_sectionDao_throw_SectionDoesntExistException
	 * () throws GameDoesntExistException, SectionDoesntExistException { GameDto
	 * gameDto = mock(GameDto.class);
	 * 
	 * when(gameDao.get(GAME_ID)).thenReturn(gameDto);
	 * when(sectionDao.getAvailable(GAME_ID, SECTION_NAME)).thenThrow(new
	 * SectionDoesntExistException());
	 * 
	 * paymentService.getPayableItemsViewModel(chooseTicketsVM); }
	 * 
	 * @Test(expected = GameDoesntExistException.class) public void
	 * given_a_ChooseTicketsViewModel_getPayableItemsViewModel_should_throw_GameDoesntExistException_when_gameDao_throw_GameDoesntExistException
	 ...
	 * paymentService.getPayableItemsViewModel(chooseTicketsVM); }
	 * 
	 * @Test public void saveToCart_should_save_choose_tickets_to_current_cart()
	 * throws GameDoesntExistException, SectionDoesntExistException { GameDto
	 * gameDto = mock(GameDto.class); SectionDto sectionDto =
	 * mock(SectionDto.class);
	 * 
	 * when(gameDao.get(GAME_ID)).thenReturn(gameDto);
	 * when(sectionDao.getAvailable(GAME_ID,
	 * SECTION_NAME)).thenReturn(sectionDto);
	 * when(sectionDto.isGeneralAdmission()).thenReturn(true);
	 * when(calculator.calculateCumulativePriceForGeneralAdmission
	 * (any(Integer.class), any(Double.class))).thenReturn( CUMULATIVE_PRICE);
	 * 
	 ...
	 * 
	 * @Test(expected = GameDoesntExistException.class) public void
	 * saveToCart_should_raise_GameDoesntExistException_when_gameDao_raise_GameDoesntExistException
	 * () throws GameDoesntExistException, SectionDoesntExistException {
	 * SectionDto sectionDto = mock(SectionDto.class);
	 * when(sectionDao.get(GAME_ID, SECTION_NAME)).thenReturn(sectionDto);
	 * 
	 ...
	 * 
	 * @Test(expected = SectionDoesntExistException.class) public void
	 * saveToCart_should_raise_SectionDoesntExistException_when_sectionDao_raise_SectionDoesntExistException
	 * () throws GameDoesntExistException, SectionDoesntExistException { GameDto
	 * gameDto = mock(GameDto.class);
	 * when(gameDao.get(GAME_ID)).thenReturn(gameDto);
	 * 
	 ...
	 * 
	 * @Test public void
	 * getCreditCardTypes_should_return_the_a_list_of_creditCardType() {
	 ...
	 * 
	 * @Test public void
	 * getCumulativePriceFR_should_return_the_FR_price_when_currentCart_contains_tickets
	 ...
	 * 
	 * @Test(expected = NoTicketsInCartException.class) public void
	 * getCumulativePriceFR_should_throw_NoTicketsInCartException_when_currentCart_contains_no_tickets
	 * () throws NoTicketsInCartException {
	 ...
	 * 
	 * @Test public void
	 * given_a_paymentViewModel_buyTicketsInCart_should_call_pay_of_a_credit_card
	 * () throws InvalidCreditCardException, NoTicketsInCartException {
	 * PaymentViewModel paymentVM = new PaymentViewModel(); MisterCard
	 * creditCard = mock(MisterCard.class);
	 * 
	 ...
	 * 
	 * @Test public void
	 * buyTicketsInCart_should_make_tickets_unavailable_to_other_people() throws
	 * InvalidCreditCardException, NoTicketsInCartException { PaymentViewModel
	 * paymentVM = new PaymentViewModel(); MisterCard creditCard =
	 * mock(MisterCard.class);
	 * 
	 * when(creditCardFactory.createCreditCard(paymentVM)).thenReturn(creditCard)
	 * ; when(currentCart.containTickets()).thenReturn(true);
	 * when(currentCart.getCumulativePrice()).thenReturn(CUMULATIVE_PRICE);
	 ...
	 * 
	 * @Test public void emptyCart_should_empty_currentCart() {
